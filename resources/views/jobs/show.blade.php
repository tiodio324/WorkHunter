    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <section class="md:col-span-3">
            ...
            <div class="container mx-auto p-4">
                ...
                @auth
                    @can('apply', $job)
                        <h2 class="text-xl my-5 font-semibold mb-4">Put "Job Application" as the subject of your email and attach your resume.</h2>

                        <div x-cloak x-data="{open: false}" id="applicant-form">
                            ...
                            <div x-show="open" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
                                <div @click.away="open = false" class="bg-white p-6 rounded-lg shadow md w-full max-w-md">
                                    ...
                                    <form method="POST" action="{{route('applicant.store', $job->id)}}" enctype="multipart/form-data">
                                        ...
                                        <x-inputs.text id="contact_email" type="email" name="contact_email" label="Contact Email" :required="true" />
                                        ...
                                        <button @click="open = false" type="button" class="bg-gray-300 hover:bg-gray-300 text-black px-4 py-2 rounded-md">
                                            Cancel
                                        </button>
                                    </form>
                                </div>
        ...
        <aside class="bg-white rounded-lg shadow-md p-3">
            ...
            @guest
                <p class="mt-10 bg-gray-200 text-gray-700 font-bold w-full py-2 px-4 rounded-full text-center">
                    <i class="fas fa-info-circle mr-3"></i> You Must be logged in for save job in to your saved jobs.
                </p>
            @else
                <form
                    method="POST"
                ...
                >
                    ...
                    @if (auth()->user()->shoppingCartJobs()->where('job_id', $job->id)->exists())
                        ...
                        <button type="submit" class="bg-red-500 hover:bg-red-600 text-white font-bold w-full py-2 px-4 rounded-full flex items-center justify-center">
                            <i class="fas fa-bookmark mr-3"></i> Remove From Saved Jobs
                        </button>
                    @else
                        <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-bold w-full py-2 px-4 rounded-full flex items-center justify-center">
                            <i class="fas fa-bookmark mr-3"></i> Save Job
                        </button>
                    @endif
                </form>
            @endguest
...
<script>
    ...
    function init(){
        ...
        ymaps.geocode(encodedAddress).then(function (res) {
            // console.log('res: ', res);
